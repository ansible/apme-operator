...
- **Pod security**: manager and operand pods target restricted / arbitrary UID
  (UBI minimal, `runAsNonRoot`, drop ALL caps, no privileged).
...
- **Makefile-only gates**: use `make lint` / `make test` in docs and CI, not
...
3. **Fix & Validate Locally:** Fix all issues in minimal commits. **Crucially, run local validation before pushing** (e.g., `make lint` or `make test`) to ensure your fix doesn't break something else.
